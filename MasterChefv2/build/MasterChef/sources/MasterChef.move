...
/// functions list for only owner ///
    ...
    public entry fun set<CoinType>(
        ...
        pool_info.fee = if (FEE_LIMIT > fee) {
            fee
        } else {
            FEE_LIMIT
        }
    ...
    public entry fun withdraw_dev_fee<CoinType>(dev_account: &signer) acquires MasterChefData, PoolInfo {
        ...
        assert!(coin::balance<CoinType>(RESOURCE_ACCOUNT_ADDRESS) > (pool_info.total_real_share as u64), ERR_INSUFFICIENT_BALANCE);

        let resource_account_signer = get_resource_account_signer();
        let amount_out = coin::balance<CoinType>(RESOURCE_ACCOUNT_ADDRESS) - (pool_info.total_real_share as u64);
...
/// functions list for every user ///
    ...
    public entry fun deposit<CoinType>(
        ...
        if (amount > 0) {
            ...
            pool_info.total_real_share = pool_info.total_real_share + (amount_in as u128);
    ...
    public entry fun withdraw<CoinType>(
        ...
        user_info.reward_debt = (user_info.amount as u128) * pool_info.acc_reward_per_share / ACC_REWARD_PRECISION;
        user_info.reward_debt = user_info.reward_debt * multiplier / (PERCENT_PRECISION as u128);
        pool_info.total_real_share = pool_info.total_real_share - (amount_out as u128);
